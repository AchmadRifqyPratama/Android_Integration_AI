...
def kalman_filter(z_ppg1,z_ppg2,xposter,Pposter):
    ...
    return xposteriori, xposteriori, Pposteriori
    
def main(fin, ind, i):
    filename = join(dirname(__file__), "WESAD_S3.csv")
    ...
    ppgyy = scipy.signal.detrend(ppgx, type = 'linear')*10
    accx=AX[ind:fin]
    accy=AY[ind:fin]
    accz=AZ[ind:fin]
    ...
    if i < 100:
        z_ppg1[i] = p1
        z_ppg2[i] = p2
        xposter=(z_ppg1[0]+z_ppg2[0])/2
        Pposter = 0
        result = kalman_filter(z_ppg1[0:i+1], z_ppg2[0:i+1], xposter, Pposter)
        heartbeat = result[0]
        xposter = result[1]
        Pposter = result[2]
        return heartbeat
        ...
        xposter = (z_ppg1[0] + z_ppg2[0])/2
        Pposter = 0
        result = kalman_filter(z_ppg1, z_ppg2, xposter, Pposter)
        heartbeat = result[0]
        xposter = result[1]
        Pposter = result[2]
        return heartbeat